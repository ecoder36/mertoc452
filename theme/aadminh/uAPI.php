...
function users_add($name,$uname,$password)
{
   ...
   if (($n_isadmin != 0) && ($n_isadmin != 1))
    ...
    $query = sprintf("INSERT INTO `users` VALUE(NULL,'%s','%s','%s','%d')",$n_name,$n_uname,$n_pass,$n_isadmin);
    //echo $query;
    $qresult = @mysql_query($query);
    if (!$qresult)
        return false;
...
function users_update($uid,$name = null,$uname = null,$password = null,$isadmin = -1)
{
    global $tf_handle;
    $id = (int)$uid;
    if($id == 0)
