...
// $result = $mysqli->query('SELECT * , COUNT(percentage) AS "percentage" FROM googlechart WHERE `weekly_Task` NOT LIKE "%leep%" GROUP BY weekly_Task ; ');
 $result = $mysqli->query('SELECT * , COUNT(type) AS "atype" FROM record WHERE `name` NOT LIKE "%leep%" GROUP BY dept ; ');
  ...
  ?>
            var chart = AmCharts.makeChart("dashboard_amchart_4", {
                ...
                "dataProvider": [{
                  ...
                  //  "country": "Germany",
                  //  "value": 65
                    "country":"<?php echo $user->name ; ?>" ,
                    "value": <?php echo  $user->id ;  ?>
                }, {
                  <?php for($ai = 0 ; $ai < $ucount; $ai++)
                    {
                        $userq = $users[$ai];
 ?>
                    "country":"<?php echo $userq->type ; ?>" ,
                    "value": <?php echo  count($userq->type) ; } ?>
                },
